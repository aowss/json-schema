...
. Check the use of the `$schema` property in xref:current-and-external-schemas[this section]
...
A `$schema` keyword footnote:[See http://json-schema.org/latest/json-schema-core.html#rfc.section.7] *can* be used to explicitly specify that a JSON document is a schema.
...
The `format` keyword footnote:[See http://json-schema.org/latest/json-schema-validation.html#rfc.section.7] can be used to define specific formats. +
...
* `regex` footnote:[This should be a valid http://www.ecma-international.org/publications/files/ECMA-ST/Ecma-262.pdf[ECMA 262] regular expression]
